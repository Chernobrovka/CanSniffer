...
from src.sharedcomponets import ShowdatagramsViewTable
    ...
    def data(self, qIndex, role=None):
        ...
        if role == Qt.DisplayRole:
            ...
            if qIndex.column() == 0:
                ...
                type_code = datagram.getDatagramTypeCode()
                
                if type_code in ['T', 'R']:  # Extended frames
                    # Extended ID (29-bit) - 8 hex цифр
                    return f"0x{datagram.messageID:08X}"
                elif type_code in ['t', 'r']:  # Standard frames
                    # Standard ID (11-bit) - 3 hex цифры, маска 0x7FF
                    std_id = datagram.messageID & 0x7FF
                    return f"0x{std_id:03X}"
                else:
                    # По умолчанию для NautilusDatagram (всегда extended)
                    return f"0x{datagram.messageID:08X}"
